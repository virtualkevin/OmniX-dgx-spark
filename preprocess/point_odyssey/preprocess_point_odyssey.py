...
def process_sequence(seq_dir, out_seq_dir):
    ...
    out_traj_dir = os.path.join(out_seq_dir, "traj")
    ...
    try:
        if not (len(cam_ints) == len(trajs_3d) == len(cam_exts) == len(rgbs) == len(depths)):
            raise ValueError(
    ...
    for i in tqdm(range(len(cam_exts)), desc="Processing frames", leave=False):
        ...
        np.savez(
            os.path.join(out_traj_dir, basename + ".npz"), trajs_2d=trajs_2d[i], trajs_3d=trajs_3d[i], valids=valids[i], visibs=visibs[i]
        )
