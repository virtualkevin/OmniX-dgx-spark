...
class SpringDataset(BaseDataset):
    """Spring dataset, support static_image/unsynced_video/synced_video/hybrid_video
        dataset_info: dict
            num_video: 1
            video_names: [""] # empty, cause we use cut3r processed dataset
        scenes: List[dict]
            scene_info:
                scene_path: str
                num_video_frame: int
    ...
    def _load_per_image_data(self, scene_path, path_info, index_info):
        ...
        for path_info_single, index_info_single in zip(path_info, index_info):
            ...
            # load image
            image_path = osp.join(scene_path, "rgb", video_name, f"{int(image_name):04d}.png")
            image = imread_cv2(image_path)
            images.append(image)

            # load camera
            camera_path = osp.join(scene_path, "cam", video_name, f"{int(image_name):04d}.npz")
            camera_data = np.load(camera_path)
            camera_intrinsics.append(camera_data["intrinsics"])
            camera_poses.append(camera_data["pose"]) 

            # load depth
            depth_path = osp.join(scene_path, "depth", video_name, f"{int(image_name):04d}.npy")
