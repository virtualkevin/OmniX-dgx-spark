...
class Stereo4dDataset(BaseDataset):
    """Stereo4d dataset, support static_image/unsynced_video/synced_video/hybrid_video
        dataset_info: dict
            num_video: 1
            video_names: ["cam_left", ...]
        scenes: List[dict]
            scene_info:
                scene_path: str
                num_video_frame: int
    ...
    def _load_per_image_data(self, scene_path, path_info, index_info):
        ...
        for path_info_single, index_info_single in zip(path_info, index_info):
            ...
            # load image
            image_path = osp.join(scene_path, video_name, "image", f"frame_{int(image_name):04d}.png")
    ...
    def _load_per_timestamp_data(self, scene_path, path_info, index_info):
        ...
        for frame_idx in unique_frame_idxs:
            # load vertex
            trajectory_path = osp.join(scene_path, "cam_left", "trajectory", f"frame_{frame_idx:04d}.npz")
            trajectory = np.load(trajectory_path)
            trajectory_list.append(trajectory["trajectory"])
    ...
    def _get_views(self, idx, data_sampling_type, total_num_image, num_image, num_unsynced_video, \
        ...
        valid_masks = np.ones_like(images[..., 0]).astype(bool)
