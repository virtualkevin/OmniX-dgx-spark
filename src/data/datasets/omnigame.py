...
class OmniGameDataset(BaseDataset):
    """OmniGame dataset, support static_image/unsynced_video/synced_video/hybrid_video
        dataset_info: dict
            num_video: 1
            video_names: ["cam_0"]
        scenes: List[dict]
    ...
    def _load_per_image_data(self, scene_path, path_info, index_info):
        ...
        for path_info_single, index_info_single in zip(path_info, index_info):
            ...
            # load image
            image_path = osp.join(scene_path, "image", video_name, f"frame_{int(image_name):04d}.png")
            ...
            # load depth
            depth_path = osp.join(scene_path, "depth", video_name, f"frame_{int(image_name):04d}.png")
            depth = imread_cv2(depth_path, options=cv2.IMREAD_UNCHANGED).astype(np.float32) / 65535.0
            near_mask = depth < 0.0015   # 1. too close
            far_mask = depth > (65500.0 / 65535.0) # 2. filter sky
            # TODO: check
    ...
    def _get_views(self, idx, data_sampling_type, total_num_image, num_image, num_unsynced_video, \
        ...
        for idx, (image, foreground_mask, depth, camera_intrinsic) in enumerate(zip(scene_data["image"], scene_data["foreground_mask"], scene_data["depth"], scene_data["camera_intrinsic"])):
            ...
            valid_mask = (depth > 0) & np.isfinite(depth) & (depth != -1) # depth process before
