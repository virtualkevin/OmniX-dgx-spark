...
def compute_tapvid3d_metrics(
    ...
    pred_tracks = pred_tracks * scale_factor[..., None] # debug
...
#   b, t, n = 3, 7, 10
#   gt_occluded = np.random.rand(b, t, n) < 0.2
#   gt_tracks = np.random.rand(b, t, n, 3) * 10

#   pred_occluded = np.random.rand(b, t, n) < 0.2
#   pred_tracks = gt_tracks + np.random.rand(b, t, n, 3) * 0.1
#   intrinsics_params = np.zeros((b, 4))
#   intrinsics_params[:, 0] = 100
#   intrinsics_params[:, 1] = 100

#   result = compute_tapvid3d_metrics(gt_occluded, gt_tracks, pred_occluded, pred_tracks, intrinsics_params, order='b t n')
