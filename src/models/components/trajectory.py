...
class TrajectoryMod(torch.nn.Module):
    ...
    def __init__(
        ...
        self.w_dynamic_st = w_dynamic_st
        self.use_w_dynamic = use_w_dynamic
    ...
    def forward(self, pts3d, offset_output, motion_field_list=None, dynamic_token_info=None, time_info=None):
        ...
        if self.use_w_dynamic:
            w_dynamic = pts3d_dynamic_score.unsqueeze(2).unsqueeze(-1)
            # tmp_debug
            threshold = 0.5 if self.training else 0.3
            if self.w_dynamic_st:
                w_dynamic_hard = (w_dynamic > threshold).float()
                w_dynamic = w_dynamic_hard + w_dynamic - w_dynamic.detach()
