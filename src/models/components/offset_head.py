...
class OffsetDPTHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        motion_dim=9,
        use_motion_max_pooling=False,
        pred_offset_num_level=2,
        pred_conf=False,
        num_point=9,
        ...
        output_dim = motion_field_num_level * num_point + pred_offset_num_level * num_point * 2
        if pred_conf:
        ...
        # custom
        self.motion_field_num_level = motion_field_num_level
        self.motion_dim = motion_dim
        self.pred_offset_num_level = pred_offset_num_level
        ...
        self.use_motion_max_pooling = use_motion_max_pooling
        
        self.motion_proj = nn.Linear(
            self.motion_dim * self.motion_field_num_level * 2 * resize_factor * resize_factor, dim_in
        )
        if self.use_motion_max_pooling:
            self.motion_proj_mp = nn.Linear(
                self.motion_dim * resize_factor * resize_factor, dim_in // self.motion_field_num_level,
                nn.LayerNorm(dim_in // self.motion_field_num_level),
            )
            self.motion_proj_mp_merge = nn.Linear(dim_in*2, dim_in)
        self.dynamic_mask_proj = nn.Linear(
            1 * resize_factor * resize_factor, dim_in
        )
        self.feature_merge = nn.Linear(
            dim_in * 3, dim_in
        )

        # reset param
        with torch.no_grad():
            self.feature_merge.weight[:, :dim_in] = torch.eye(dim_in)
            self.feature_merge.weight[:, dim_in:] = 0
            self.feature_merge.bias.zero_()

        last_conv = self.scratch.output_conv2[-1]  # 取 Sequential 的最后一层 Conv2d
        assert isinstance(last_conv, nn.Conv2d)

        with torch.no_grad():
            # 权重用小正态分布
            nn.init.normal_(last_conv.weight, mean=0.0, std=1e-3)
            # 偏置初始化成 0.0 或者一个小值，比如 0.5，如果希望初始在中间偏移
    ...
    def forward(
        ...
        patch_start_idx: int,
        chunk_size: int = 8,
        motion_field_list=None,
        dynamic_token_info=None,
        ...
        """
        # TODO: motion_field embedding resize_factor 1x -> 2x
        B, S, N, C = feats[0][0].shape
        feats = [feat[0] for feat in feats]
        hp, wp = H // self.patch_size, W // self.patch_size
        # custom
        motion_field_over_time = calculate_mean_std_over_time(motion_field_list)
        motion_field_over_time = rearrange(motion_field_over_time, "b im (hp s1 wp s2) d -> b im (hp wp) (s1 s2 d)", hp=hp, wp=wp, s1=self.resize_factor, s2=self.resize_factor)
        motion_feature = self.motion_proj(motion_field_over_time)
      
        if self.use_motion_max_pooling:
            motion_feature_mp = [self.motion_proj_mp(motion_field) for motion_field in motion_field_list]
            motion_feature_mp = torch.cat(motion_feature_mp, dim=-1)
            motion_feature_mp = torch.max(motion_feature_mp, dim=2)[0]
            motion_feature = self.motion_proj_mp_merge(torch.cat([motion_feature, motion_feature_mp], dim=-1))

        dynamic_mask = dynamic_token_info["dynamic_mask"].unsqueeze(-1)
        dynamic_mask = rearrange(dynamic_mask, "b im (hp s1 wp s2) d -> b im (hp wp) (s1 s2 d)", hp=hp, wp=wp, s1=self.resize_factor, s2=self.resize_factor)
        dynamic_mask_feature = self.dynamic_mask_proj(dynamic_mask)

        feats = [
            torch.cat([feat, motion_feature, dynamic_mask_feature], dim=-1) for feat in feats
        ]
        feats = [
            self.feature_merge(feat) for feat in feats
        ]
    ...
    def _forward_impl(
        ...
        if self.training and self.use_checkpointing:
            fused_main = checkpoint(self._fuse, resized_feats, use_reentrant=False)
        else:
        ...
        if self.training and self.use_checkpointing:
            fused_main = checkpoint(
                lambda t: custom_interpolate(
                    t,
                    size=(h_out, w_out),
                    scale_factor=None,
                    mode="bilinear",
        ...
        out = self.scratch.output_conv2(fused_main)
        out = out.permute(0, 2, 3, 1)  # B,H,W,C expected
        if self.pred_offset_num_level > 0:
            sampling_offsets = out[..., :self.pred_offset_num_level*self.num_point*2]
            if not self.pred_conf:
                sampling_weights = out[..., self.pred_offset_num_level*self.num_point*2:]
            else:
                sampling_weights = out[..., self.pred_offset_num_level*self.num_point*2:-1]
                sampling_conf = out[..., -1]
            sampling_weights = F.softmax(sampling_weights, dim=-1)

            # sampling_offsets = sampling_offsets.reshape(B, S, *sampling_offsets.shape[1:])
            # sampling_weights = sampling_weights.reshape(B, S, *sampling_weights.shape[1:])
            if self.pred_conf:
                # sampling_conf = sampling_conf.reshape(B, S, *sampling_conf.shape[1:])
                # act expp1
                sampling_conf = 1 + torch.clamp(sampling_conf, min=-10 ,max=10).exp()
            ...
            if self.pred_conf:
                output["sampling_conf"] = sampling_conf

        else:
            if not self.pred_conf:
                sampling_weights = out
            else:
                sampling_weights = out[..., :-1]
                sampling_conf = out[..., -1]
            sampling_weights = F.softmax(sampling_weights, dim=-1)
            # sampling_weights = sampling_weights.reshape(B, S, *sampling_weights.shape[1:])
            ...
            if self.pred_conf:
                # sampling_conf = sampling_conf.reshape(B, S, *sampling_conf.shape[1:])
                # act expp1
                sampling_conf = 1 + sampling_conf.exp()
...
### custom
def calculate_mean_std_over_time(motion_feature_list):
    motion_feature = torch.cat(motion_feature_list, dim=-1)
    motion_feature_mean = torch.mean(motion_feature, dim=2)
    motion_feature_std = torch.std(motion_feature, dim=2)
    motion_feature_over_time = torch.cat([
        motion_feature_mean, motion_feature_std
    ], dim=-1)

    return motion_feature_over_time
