...
def validate_options(
    ...
    if (
        options.candidate_source_view_count is not None
        and options.candidate_source_view_count <= 0
    ):
        raise ConversionError(
            "invalid_candidate_source_views",
            "Candidate source-view count must be positive.",
            status_code=400,
        )
    return ConversionOptions(
        ...
        candidate_source_view_count=options.candidate_source_view_count,
    )
