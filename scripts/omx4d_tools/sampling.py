...
def sample_predictions(
    ...
    *,
    ...
    dynamic_reserved_fraction: float = 0.8,
    candidate_source_view_count: int | None = None,
    ...
    if candidate_source_view_count is None:
        candidate_source_view_count = predictions.source_view_count
    if not 1 <= candidate_source_view_count <= predictions.source_view_count:
        raise ConversionError(
            "invalid_candidate_source_views",
            "Candidate source-view count must be within the prediction source views.",
        )
    ...
    # Tail chunks repeat their last real input frame. Excluding those padded
    # source views prevents the 500k budget from being spent on duplicate
    # reference pixels while retaining all target trajectory frames.
    valid[candidate_source_view_count * predictions.height * predictions.width :] = False
    ...
    sampling: dict[str, object] = {
        ...
        "dynamicScoreCutoff": (
            float(flat_score[dynamic_selected].min().item())
            if dynamic_take
            else None
        ),
        "dynamicRanking": "global-descending-stable-identity-tiebreak",
        "spatialSelectedPointCount": int(spatial_selected.numel()),
        "spatialDistribution": "normalized-frame-zero-3d-voxel",
        "candidateSourceViewCount": candidate_source_view_count,
        "excludedPaddedSourceViewCount": (
            predictions.source_view_count - candidate_source_view_count
        ),
